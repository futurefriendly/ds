define('didimonitor', function(require, exports, module){ (function(root, factory) {
    ...
    var windowLoadManager = (function() {
        ...
        function compatibleWindowLoad(cb) {
            if(!windowLoaded){
                logList.push(cb); // 加入回调队列
            } else {
                cb(); // 直接回调
            }
        }
        return {
            onload: compatibleWindowLoad
        }
    })();
    ...
    function _sendH5BeatlesStat(action, params, instantly/*optional*/){
        ...
        var url = '//api.didialift.com/beatles/h5/user/log/onlinelog',
            _params,
            ...
            // 删除?(%3F)之后的字符
            _channel = (_getQuery('regfrom') || _getQuery('channel')).replace(/\?.*/g, '');
     ...
     * 所有发送在onload事件之后发送
     */
    function _sendH5BeatlesStatAfterOnload(action, params, instantly/*optional*/) {
        windowLoadManager.onload(function() {
            _sendH5BeatlesStat(action, params, instantly);
        });
    }
    ...
    $.extend(DidiMonitor, {
        ...
        , sendBeatles: _sendH5BeatlesStatAfterOnload

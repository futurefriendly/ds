define('didi-component-dd/dialog.js', function(require, exports, module){ /**
 * 生成对话框dd.dialog 不依赖其他库but dd.ui.css
 */

// 需要将dialog注册到dd命名空间下
var dd = module.exports;
...
var util = {
	...
	genTip: function(opts) {
		...
		if (title.txt) {
			...
			tip.size = tip.size || '.875rem'
		} else {
			tip.color = tip.color || "#333";
			tip.size = tip.size || '1rem';
		}
	...
	genClose: function(close) {
		if(close){
			var cssText = close.cssText || "";
			return '<a class="close" href="javascript:void(0);" style="' + cssText + '"></a>';
		}else{
			return '';
		}
	},
	...
	genButtons: function(btns, ext) {
		...
		if (btns && this.isArray(btns)) {
			...
			for (var i = 0, btn = null, l = btns.length; i < l; i++) {
				btn = btns[i];
				res += '<span class="'+ btn.kls +'"><a id="'+ btn.id +'">'+ btn.val +'</a></span>';
			}
			res += '</div>';
		}
		//按钮下面附加内容
